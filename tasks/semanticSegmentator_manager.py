...
class SemanticSegmentation_Manager():
    def __init__(self, cf, model):
        ...
        self.validator = self.validation(self.logger_stats, self.model, cf)
        self.trainer = self.train(self.logger_stats, self.model, cf, self.validator)
        self.predict = self.predict(self.logger_stats, self.model, cf)

    class train():
        def __init__(self, logger_stats, model, cf, validator):
        ...
        def start(self, criterion, optimizer, train_loader, train_set, valid_set=None, valid_loader=None, scheduler=None):
            ...
            for epoch in range(self.curr_epoch, self.cf.epochs + 1):
                ...
                if valid_set is not None and valid_loader is not None:
                    ...
                    val_loss, acc_cls, mean_IoU = self.validator.start(criterion, valid_set, valid_loader, epoch)
                ...
                if self.best_IoU < mean_IoU:
                    self.msg_stats_best = 'Best case: epoch = %d, mIoU = %.2f, loss = %.5f\n' % (epoch, 100*mean_IoU,val_loss)
    ...
    class validation():
        def __init__(self, logger_stats, model, cf):
            ...
            self.model = model
            self.cf = cf
            self.TP_list, self.FP_list, self.FN_list = np.zeros(self.cf.num_classes), np.zeros(self.cf.num_classes), \
            np.zeros(self.cf.num_classes)
        ...
        def start(self, criterion, valid_set, valid_loader, epoch=None):
            ...
            for vi, data in enumerate(valid_loader):
                ...
                # Compute batch stats
                self.val_loss.update(criterion(outputs, gts).data[0] / n_images, n_images)
                TP, FP, FN = compute_stats(predictions, gts.cpu().data.numpy(), self.cf.num_classes, self.cf.void_class)
                self.TP_list = map(operator.add, self.TP_list, TP)
                self.FN_list = map(operator.add, self.FN_list, FN)
                self.FP_list = map(operator.add, self.FP_list, FP)

            # Compute mean stats
            mean_IoU = compute_mIoU(self.TP_list, self.FP_list, self.FN_list)
            acc = 0  # np.mean(acc)
            ...
            if epoch is not None:
                ...
                self.logger_stats.write('[epoch %d], [val loss %.5f], [acc %.2f], [acc_cls %.2f], [mean_IoU %.2f], [fwavacc %.2f]' % (
                    epoch, self.val_loss.avg, 100*acc, 100*acc_cls, 100*mean_IoU, 100*fwavacc))
                self.logger_stats.write('---------------------------------------------------------------- \n')
            else:
                self.logger_stats.write('----------------- Scores summary --------------------')
                self.logger_stats.write('[val loss %.5f], [acc %.2f], [acc_cls %.2f], [mean_IoU %.2f], [fwavacc %.2f]' % (
                    self.val_loss.avg, acc, 100*acc_cls, 100*mean_IoU, 100*fwavacc))
                self.logger_stats.write('---------------------------------------------------------------- \n')
            return self.val_loss.avg, acc_cls, mean_IoU
