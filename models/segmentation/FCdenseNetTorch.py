...
class FCDenseNet(Model):
    ...
    def __init__(self, nb_layers_per_block, growth_rate, nb_dense_block=5, n_channel_start=48,
        ...
        self.nb_layers_per_block = [nb_layers_per_block] * \
            (2 * nb_dense_block + 1) if type(nb_layers_per_block) == int else nb_layers_per_block
        self.growth_rate = [growth_rate] * (2 * nb_dense_block + 1) if \
            type(growth_rate) == int else growth_rate
        self.drop_rate = drop_rate
        assert len(self.nb_layers_per_block) == len(self.growth_rate) == 2 * nb_dense_block + 1
        ...
        for i in range(nb_dense_block):
            self.dense_blocks.append(
                DenseBlock(nIn, self.growth_rate[i], self.nb_layers_per_block[i],
                           drop_rate=drop_rate, bottle_neck=bottle_neck))
            nIn += self.growth_rate[i] * self.nb_layers_per_block[i]
            nskip.append(nIn)
            self.transition_downs.append(self.get_TD(nIn, drop_rate))

        self.dense_blocks.append(
            DenseBlock(nIn, self.growth_rate[nb_dense_block], self.nb_layers_per_block[nb_dense_block],
                       only_new=True, drop_rate=drop_rate,
                       bottle_neck=bottle_neck))
        nIn = self.growth_rate[nb_dense_block] * self.nb_layers_per_block[nb_dense_block]
        ...
        for i in range(nb_dense_block-1):
            ...
            nIn += nskip.pop()
            self.dense_blocks.append(
                DenseBlock(nIn, self.growth_rate[nb_dense_block + 1 + i],
                           self.nb_layers_per_block[nb_dense_block + 1 + i],
                           only_new=True, drop_rate=drop_rate,
                           bottle_neck=bottle_neck))
            nIn = self.growth_rate[nb_dense_block + 1 + i] * \
                self.nb_layers_per_block[nb_dense_block + 1 + i]
        # last dense block
        self.transition_ups.append(nn.ConvTranspose2d(
            nIn, nIn, 3, stride=2, padding=1, bias=True))
        nIn += nskip.pop()
        self.dense_blocks.append(
            DenseBlock(nIn, self.growth_rate[2 * nb_dense_block],
                       self.nb_layers_per_block[2 * nb_dense_block], drop_rate=drop_rate,
                       bottle_neck=bottle_neck))
        nIn += self.growth_rate[2 * nb_dense_block] * \
            self.nb_layers_per_block[2 * nb_dense_block]
    ...
    def forward(self, x):
        ...
        skip_connects = []
        # down sample
        for i in range(self.nb_dense_block):
            x = self.dense_blocks[i](x)
            skip_connects.append(x)
            x = self.transition_downs[i](x)
        # bottle neck
        x = self.dense_blocks[self.nb_dense_block](x)
        # up sample
        for i in range(self.nb_dense_block):
            ...
            x = self.dense_blocks[self.nb_dense_block + 1 + i](x)
        x = self.conv_last(x)
